...
def main():
    ...
    for directory in dir_list:
            ...
            if dir_db is not None and directory in dir_db:
                ...
                if file_db is not None and len(file_db) > 0:
                    for file in file_db:
                        ...
                        revision = db_controller.select_file_max_rev(TABLE.FILES, directory, data_manager.parse_path(file))
                        db_controller.update_file(TABLE.DIR, TYPE.INVALID, directory, data_manager.parse_path(file), revision)
                continue
        ...
        if valid == TYPE.VALID:
            ...
            file_os = os.listdir(f"{PATH.MARKDOWN}/{directory}")
            ...
            for file in file_list:
                ...
                if not verified:
                    ...
                    if revision is not None:
                        db_controller.update_file(TABLE.FILES, TYPE.INVALID, directory, file, revision)
                    if file == PATH.IMG:
                        try:
                            source_directory = f"{PATH.MARKDOWN}/{directory}/{PATH.IMG}"
                            destination_directory = f"{PATH.HTMLS}/{directory}/{PATH.IMG}"
                            shutil.copytree(source_directory, destination_directory)
                            info_logger.info(f"Directory '{source_directory}' and its contents copied to '{destination_directory}' successfully.")
                        except FileExistsError:
                            info_logger.info(f"Error: Destination directory '{destination_directory}' already exists.")
                            error_logger.error(f"Error: Destination directory '{destination_directory}' already exists.")
                ...
                if valid == TYPE.NEW:
                    ...
                    if completed:
                        ...
                        db_controller.insert_new_file(TABLE.FILES, store.cover, store.number, store.revision, store.created, store.revised, store.title, store.file_name)
        ...
        for directory in dir_db:
            ...
            for file in file_list:
                ...
                if verified == TYPE.VERIFIED:
                    db_controller.finish_off_numbering(TABLE.FILES, file_count, cd, data_manager.parse_path(file))
                    file_count += 1
